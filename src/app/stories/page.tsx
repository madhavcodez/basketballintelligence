  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <span className="text-xs text-chrome-medium truncate max-w-[140px]">{label}</span>
        <span className="text-xs font-semibold text-chrome-light">
          {value.toFixed(1)}{suffix ?? ''}
        </span>
      </div>
      <svg viewBox="0 0 200 8" className="w-full h-2" role="img">
        <rect x="0" y="0" width="200" height="8" rx="4" fill="rgba(255,255,255,0.06)" />
        <motion.rect
          x="0"
          y="0"
          height="8"
      ...
      <GlassCard className="p-0 overflow-hidden" tintColor={config.tintColor} hoverable>
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-lg sm:text-xl font-bold tracking-tight text-chrome-light font-display group-hover:text-white transition-colors">
                {config.title}
              </h3>
              <p className="mt-1 text-sm text-chrome-dim line-clamp-2">{config.subtitle}</p>
              ...
              {keyStatDisplay && (
                <div className="mt-3">
                  ...
                  <span className="ml-2 text-xs text-chrome-dim uppercase tracking-wider">
            ...
            <div className="shrink-0 pt-1">
              {expanded ? (
                <ChevronUp size={20} className="text-chrome-dim" />
              ) : (
                <ChevronDown size={20} className="text-chrome-dim" />
              )}
            </div>
          </div>
        </button>
        ...
        <AnimatePresence>
          {expanded && (
            ...
            >
              <div className="px-5 sm:px-6 pb-5 sm:pb-6 border-t border-glass-border pt-4">
                {loading ? (
                  <div className="flex items-center justify-center py-8">
                    <Loader2 size={24} className="animate-spin text-chrome-dim" />
                    <span className="ml-2 text-sm text-chrome-dim">Loading story data...</span>
                  </div>
                ) : (
                ...
                <div className="mt-4 flex justify-end">
                  <button
                    ...
                    className={clsx(
                      ...
                      copied ? 'text-accent-green border-accent-green/30' : 'text-chrome-dim',
                    )}
                  >
...
function StoryContent({
  ...
  if (!data) {
    return <p className="text-sm text-chrome-dim">Unable to load story data. Please try again.</p>;
  }
...
function ThreePointStory({
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-chrome-medium leading-relaxed">
      ...
      {currySeasons.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">Curry&apos;s 3PA Per Game by Season</h4>
      ...
      {allTimeScorers.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">All-Time Scoring Leaders</h4>
          <div className="space-y-1.5">
            {allTimeScorers.slice(0, 5).map((scorer, i) => (
              <div key={scorer.name} className="flex items-center gap-3 text-xs">
                <span className="w-5 text-right text-chrome-dim font-mono">{i + 1}</span>
                <span className="flex-1 text-chrome-light font-medium">{scorer.name}</span>
                <span className="text-chrome-medium font-semibold">{Number(scorer.value).toLocaleString()}</span>
                {scorer.active === 'Y' && (
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-accent-green/10 text-accent-green font-semibold">
                    ACTIVE
                  </span>
                )}
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {restrictedAreaStats.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">Restricted Area FG% Leaders</h4>
            ...
            {restrictedAreaStats.map((player) => (
              <div key={player.name} className="space-y-1">
                <StoryBar
                  ...
                  suffix="%"
                />
                <div className="text-[10px] text-chrome-dim pl-0">
      ...
      {restrictedAreaStats.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">Volume: Attempts in Restricted Area</h4>
          <div className="space-y-2">
      ...
      {restrictedAreaStats.length === 0 && (
        <p className="text-sm text-chrome-dim italic">Shot zone data not available for these players.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      {rookieSeasons.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">Rookie Season Scoring</h4>
          <div className="space-y-2">
              ...
              <div key={rookie.name} className="space-y-1">
                ...
                {rookie.roy && (
                  <div className="flex items-center gap-1 ml-0">
                    <Trophy size={10} className="text-accent-gold" />
                    <span className="text-[10px] text-accent-gold font-semibold">ROY</span>
                  </div>
                )}
              </div>
            ))}
      ...
      {rookieSeasons.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">Rookie Stat Comparison</h4>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {rookieSeasons.map((r) => (
              <div
                key={`chip-${r.name}`}
                className="flex flex-col gap-0.5 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]"
              >
                <span className="text-[10px] text-chrome-dim uppercase tracking-wider truncate">
                  {r.name.split(' ').pop()}
                </span>
                <span className="text-sm font-bold text-chrome-light font-display">{r.ppg} / {r.rpg} / {r.apg}</span>
                <span className="text-[10px] text-chrome-dim">{r.games} games</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {rookieSeasons.length === 0 && (
        <p className="text-sm text-chrome-dim italic">Rookie data not available.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      {mvpProfiles.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">Peak Season MVP Score</h4>
          <div className="space-y-2">
            {mvpProfiles.map((mvp) => (
              <StoryBar
                key={mvp.name}
      ...
      {mvpProfiles.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold text-chrome-light mb-3">MVP Winners - Peak Stats</h4>
          <div className="overflow-x-auto">
            <table className="w-full text-xs min-w-[400px]">
              <thead>
                <tr className="border-b border-glass-border">
                  <th className="text-left py-2 px-2 text-chrome-dim font-semibold uppercase tracking-wider">Player</th>
                  <th className="text-right py-2 px-2 text-chrome-dim font-semibold uppercase tracking-wider">MVPs</th>
                  <th className="text-right py-2 px-2 text-chrome-dim font-semibold uppercase tracking-wider">Peak PPG</th>
                  <th className="text-right py-2 px-2 text-chrome-dim font-semibold uppercase tracking-wider">RPG</th>
                  <th className="text-right py-2 px-2 text-chrome-dim font-semibold uppercase tracking-wider">APG</th>
                  <th className="text-right py-2 px-2 text-chrome-dim font-semibold uppercase tracking-wider">Season</th>
                </tr>
              </thead>
              <tbody>
                {mvpProfiles.map((mvp) => (
                  <tr key={mvp.name} className="border-b border-glass-border/50">
                    <td className="py-2 px-2 text-chrome-light font-medium">{mvp.name}</td>
                    <td className="py-2 px-2 text-right">
                      {mvp.mvpCount > 0 ? (
                        <span className="text-accent-gold font-bold">{mvp.mvpCount}</span>
                      ) : (
                        <span className="text-chrome-dim">0</span>
                      )}
                    </td>
                    <td className="py-2 px-2 text-right text-chrome-light font-semibold">{mvp.peakPPG.toFixed(1)}</td>
                    <td className="py-2 px-2 text-right text-chrome-medium">{mvp.peakRPG.toFixed(1)}</td>
                    <td className="py-2 px-2 text-right text-chrome-medium">{mvp.peakAPG.toFixed(1)}</td>
                    <td className="py-2 px-2 text-right text-chrome-dim">{mvp.peakSeason}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {mvpProfiles.length === 0 && (
        <p className="text-sm text-chrome-dim italic">MVP data not available.</p>
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <motion.div variants={cardVariants} className="text-center mb-2">
        <p className="text-xs uppercase tracking-[0.16em] text-accent-orange font-semibold mb-2">Data Stories</p>
        <h1 className="text-3xl sm:text-5xl font-extrabold tracking-[-0.02em] text-chrome-light font-display">
          Story Studio
        </h1>
        <p className="mt-3 text-sm sm:text-base text-chrome-dim max-w-xl mx-auto">
      ...
      <motion.div variants={cardVariants}>
        <GlassCard className="p-6 sm:p-8 shadow-[0_0_40px_rgba(255,107,53,0.06)]" tintColor="#FF6B35">
          <div className="flex items-start gap-4">
            <div className="shrink-0 flex items-center justify-center h-14 w-14 rounded-2xl bg-accent-orange/[0.12] border border-accent-orange/20">
              <BookOpen size={26} className="text-accent-orange" />
            </div>
            <div>
              <h2 className="text-xl sm:text-2xl font-extrabold tracking-[-0.02em] text-chrome-light font-display">
                Explore Stories
              </h2>
              <p className="mt-1.5 text-sm text-chrome-dim leading-relaxed max-w-lg">
                ...
                {STORY_CONFIGS.map((c) => {
                  ...
                  return (
                    <a
                      key={c.id}
                      href={`#${c.id}`}
                      className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-white/[0.05] border border-white/[0.08] text-chrome-dim hover:text-chrome-medium hover:border-white/[0.16] transition-all"
                    >
